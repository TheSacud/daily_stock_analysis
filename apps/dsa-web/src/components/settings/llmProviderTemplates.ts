...
export const LLM_PROVIDER_CAPABILITY_LABELS: Record<LLMProviderCapability, { label: string; hint: string }> = {
  ...
  'official-api': {
    label: 'Official API',
    hint: 'Uses the provider official protocol or official compatible endpoint.',
  },
  'model-discovery': {
    label: 'Model Discovery',
    hint: 'Can try to fetch the model list through /models; actual results still depend on account permissions and API key.',
  },
  vision: {
    label: 'Vision Hint',
    hint: 'The template indicates this provider is commonly used for vision scenarios; exact capabilities still depend on the account and model list.',
  },
  'local-runtime': {
    label: 'Local Runtime',
    hint: 'Requires the current runtime environment to reach the corresponding local service.',
  },
};
...
export const LLM_PROVIDER_TEMPLATES: LLMProviderTemplate[] = [
  ...
  },
  {
    ...
    configHint:
      'The same ANSPIRE_API_KEYS value can be reused for search and LLM channels. The models and gateway below are configuration examples; actual availability depends on account permissions and the console. Use Test Connection first.',
    officialSources: [
  ...
  },
  {
    ...
    capabilities: ['openai-compatible'],
    configHint: 'Confirm the online inference endpoint/region is not mixed with Coding Plan dedicated endpoints.',
    officialSources: [
  ...
  },
  {
    ...
    capabilities: ['openai-compatible', 'model-discovery'],
    configHint: 'The model list and model visibility depend on account permissions and API key.',
  ...
  },
  {
    ...
    configHint: 'Requires the local machine, Docker, or self-hosted runner to reach the Ollama service.',
