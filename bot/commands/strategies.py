...
class StrategiesCommand(BotCommand):
    ...
    def aliases(self) -> List[str]:
        return ["skills", "strategy", "strategy\u5217\u8868"]
    ...
    def execute(self, message: BotMessage, args: List[str]) -> BotResponse:
        ...
        try:
            ...
            if not all_skills:
                return BotResponse.text_response("📋 no strategies available.\u8bf7\u68c0check strategies/ \u76ee\u5f55.")

            skills = all_skills
            if show_active_only:
                skills = [s for s in all_skills if s.name in configured_active]
                if not skills:
                    return BotResponse.text_response("📋 no active strategy.")
            ...
            lines.append(f"\u5171 {total_count} \u4e2astrategy; \u5df2\u6fc0\u6d3b {active_count} \u4e2a")
            lines.append(f"\n💡 \u4f7f\u7528 `/ask <stock code> <strategy\u540d>` \u6307\u5b9astrategyanalyze")
            ...
            return BotResponse.text_response(f"⚠️ failed to fetch strategy list: {str(e)}")
