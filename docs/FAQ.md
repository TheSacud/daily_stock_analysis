...
> Full details: [LLM Config Guide](LLM_CONFIG_GUIDE.md).
...
**Pitfall**: Do not use `OPENAI_BASE_URL` for Ollama, or the system will concatenate URLs incorrectly (e.g. 404, `api/generate/api/show`). See [LLM Config Guide](LLM_CONFIG_GUIDE.md) Example 4 and channel examples.
...
> See [LLM Config Guide → Example 4 (Ollama)](LLM_CONFIG_GUIDE.md#example-4-ollama) for a complete configuration example.
...
1. Check [Complete Configuration Guide](full-guide.md)
