...
Extract stock codes from screenshots/images using a Vision LLM.
...
EXTRACT_PROMPT = """Analyze this stock-market screenshot or image and extract every visible stock code and name.
...
def _verify_image_magic_bytes(image_bytes: bytes, mime_type: str) -> None:
    ...
    if mime_type not in _IMAGE_SIGNATURES:
        raise ValueError(f"Unable to validate type: {mime_type}")
    if mime_type == "image/webp":
        if image_bytes[:4] != b"RIFF" or image_bytes[8:12] != b"WEBP":
            raise ValueError("File content does not match declared type image/webp and may be tampered with")
        return
    for sig in _IMAGE_SIGNATURES[mime_type]:
        if image_bytes.startswith(sig):
            return
    raise ValueError(f"File content does not match declared type {mime_type} and may be tampered with")
...
def _parse_codes_from_text(text: str) -> List[str]:
    """Parse stock codes from LLM response text, legacy format."""
    seen: set[str] = set()
    result: List[str] = []

    # Prefer JSON arrays; only remove the opening Markdown fence to avoid find("```") deleting the closing fence and clearing content.
    cleaned = text.strip()
    for start in ("```json", "```"):
    ...
    # Fallback: find 5-6 digit codes and US tickers.
...
def _parse_items_from_text(text: str) -> List[Tuple[str, Optional[str], str]]:
    ...
    if not codes:
        logger.info("[ImageExtractor] Could not parse structured items and legacy code extraction is empty")
...
def _call_litellm_vision(image_b64: str, mime_type: str, api_key: Optional[str] = None) -> str:
    ...
    if not model:
        raise ValueError("Vision API is not configured. Set LITELLM_MODEL or the related API key.")
    if route_has_hermes(getattr(cfg, "llm_model_list", []) or [], model):
        raise ValueError("Hermes Vision is not validated: VISION_MODEL cannot select a route that contains a Hermes deployment.")
...
def extract_stock_codes_from_image(
    ...
    """
    Extract stock codes and names from an image using a Vision LLM.

    Priority: Gemini -> Anthropic -> OpenAI, first available.
    Supports multiple-key rotation and retries, up to 3 attempts with exponential backoff.
    ...
    Returns:
        (items, raw_text) - items are [(code, name?, confidence), ...], raw_text is the raw LLM response.

    Raises:
        ValueError: raised when the image is invalid, Vision API is not configured, or extraction fails.
    """
    ...
    for attempt in range(3):
        try:
            ...
            if attempt < 2:
                delay = 2 ** attempt
                logger.warning(f"[ImageExtractor] attempt {attempt + 1}/3 failed; retrying after {delay}s: {e}")
                time.sleep(delay)
