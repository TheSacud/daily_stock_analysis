...
const QUICK_QUESTIONS = [
  ...
  { label: 'Review CATL with Elliott Wave Theory', skill: 'wave_theory' },
  { label: 'Analyze BYD trend', skill: 'bull_trend' },
  { label: 'Use range trading on SMIC', skill: 'box_oscillation' },
  { label: 'Analyze Tencent hk00700', skill: 'bull_trend' },
  { label: 'Analyze East Money with sentiment cycle', skill: 'emotion_cycle' },
];
...
const ChatPage: React.FC = () => {
  ...
  const handleToggleWatchlist = useCallback(
    async (stockCode: string) => {
      ...
      try {
        ...
        if (isMountedRef.current) {
          setWatchlistMessage('Operation failed. Please retry.');
        }
      } finally {
        if (isMountedRef.current) {
  ...
  // Handle follow-up from report page: ?stock=600519&name=Kweichow Moutai&recordId=xxx
  useEffect(() => {
    ...
    return (
      ...
      >
        ...
        <span className="flex items-center gap-1.5">
          <span className="opacity-60">Reasoning Process</span>
      ...
      <ScrollArea testId="chat-session-list-scroll" viewportClassName="p-3">
        {sessionsLoading ? (
          ...
          <DashboardStateBlock
            ...
            description="Conversation records will appear here after you start asking questions."
            className="rounded-2xl border border-dashed border-border/50 bg-surface/30"
          />
        ) : (
            ...
            {sessions.map((s) => (
              <div key={s.session_id} className="session-item-row">
                <button
                  ...
                  aria-label={`Switch to conversation ${s.title}`}
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        message="After deletion, this conversation cannot be recovered. Confirm deletion?"
        confirmText="Delete"
        cancelText="Cancel"
        isDanger
      ...
      <div className="flex h-full min-w-0 flex-1 flex-col overflow-hidden">
        <header className="mb-4 flex-shrink-0 space-y-3">
          ...
          <p className="text-secondary-text text-sm">
            Ask AI for single-stock analysis with strategy-based trading advice and real-time decision reports.
          </p>
          {sendToast ? (
            <InlineAlert
              variant={sendToast.type === 'success' ? 'success' : 'danger'}
              title={sendToast.type === 'success' ? 'Send succeeded' : 'Send failed'}
          ...
          >
            {messages.length === 0 && !loading ? (
              <div className="flex h-full items-center justify-center">
                <EmptyState
                  title="Start Asking"
                  description='Enter "Analyze 600519" or "Is Moutai a buy now?" and AI will use live data tools to generate a decision report.'
            ...
            <div className="space-y-3">
              ...
              {isFollowUpContextLoading ? (
                <InlineAlert
                  ...
                  message="Historical analysis context is loading; you can send the follow-up now."
              ...
              <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-white/6 bg-surface/25 px-3 py-2">
                ...
                >
                  ...
                  <span className="text-xs text-muted-text">Save tokens in long conversations</span>
                </label>
              ...
              {skills.length > 0 && (
                <div className="space-y-2">
                  <button
                    ...
                    aria-label={mobileSkillPickerOpen ? 'Collapse strategy selector' : 'Expand strategy selector'}
                  ...
                  >
                    <span className="flex min-w-0 items-center gap-2">
                      ...
                      <span className="flex-shrink-0 font-medium">Strategy</span>
