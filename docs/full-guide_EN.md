...
> - **Taiwan stocks**: On top of the US/HK offshore base path, the `institution` block additionally surfaces raw three major institutional investor groups net buy/sell figures (TWSE T86 / TPEx, default-on, fail-open — stays `not_supported` when data is unavailable); `capital_flow`, `dragon_tiger`, and `boards` remain `not_supported`.
...
| --- | --- | --- |
| `buy` | `strong_buy`, `strong buy`, `buy`, `position`, `build position` | `buy` |
| `add` | `add`, `increase position`, `overweight`, `accumulate` | `buy` |
| `hold` | `hold`, `keep holding`, `hold and watch`, `shakeout watch` | `hold` |
| `watch` | `watch`, `wait and see`, `wait` | `hold` |
| `reduce` | `reduce`, `trim`, `lower position` | `sell` |
| `sell` | `sell`, `exit`, `clear position`, `strong_sell`, `strong sell` | `sell` |
| `avoid` | `avoid`, `do not buy`, `avoid buying` | `hold` |
| `alert` | `alert`, `risk alert`, `caution`, `triggered alert` | `hold` |
