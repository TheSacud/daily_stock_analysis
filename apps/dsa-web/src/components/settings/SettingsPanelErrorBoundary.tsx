...
function getSafeErrorSummary(error: unknown) {
  const rawMessage = error instanceof Error
    ...
    : typeof error === 'string'
      ? error
      : 'Unknown frontend runtime error';
  const normalized = rawMessage.replace(/\s+/g, ' ').trim() || 'Unknown frontend runtime error';
...
export const SettingsPanelErrorBoundary = (props: SettingsPanelErrorBoundaryProps) => {
  ...
  const labels: SettingsPanelErrorBoundaryLabels = language === 'en'
    ...
    : {
        loadFailedSuffix: 'Load Failed',
        runtimeErrorMessage: 'A frontend runtime error occurred in this settings area. Other settings on the page remain usable.',
        defaultDiagnosticHint: 'Provide the release version, runtime environment, and entry point that triggered the issue to help diagnose it.',
        errorSummaryPrefix: 'Error summary: ',
      };
