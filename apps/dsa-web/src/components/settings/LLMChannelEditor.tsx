...
const RUNTIME_CAPABILITY_OPTIONS: Array<{ value: LLMCapabilityCheck; label: string; hint: string }> = [
  ...
  { value: 'stream', label: 'Stream', hint: 'Checks whether streaming output returns valid chunks.' },
...
const RUNTIME_ONLY_HERMES_SECRET_MESSAGE = 'Runtime-injected Hermes keys are not returned. To test in settings, re-enter the key or save it to .env.';
      ...
      {expanded ? (
        <div className="settings-surface-overlay-soft space-y-4 px-4 py-4">
          ...
          {showProviderTemplateDetails ? (
            <div className="space-y-2 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
              ...
              <p className="text-[11px] leading-5 text-muted-text">
                Capability tags are configuration references only and do not mean runtime capability has been verified.
              </p>
            </div>
          ) : null}
          ...
          <Input
            ...
            placeholder={channel.protocol === 'ollama' ? 'Local Ollama can be left empty' : 'Multiple keys are supported, comma-separated'}
          />
          </div>
          ...
          <div className="space-y-3 rounded-xl border border-[var(--settings-border)] bg-[var(--settings-surface-hover)] p-3">
            <div className="flex flex-wrap items-center gap-2">
              ...
              >
                {discoveryState?.text || 'OpenAI Compatible channels that support `/models` can fetch models automatically.'}
              </span>
            </div>
            {discoveryState?.hint ? (
            ...
            <Input
              ...
              hint={
                discoveredModels.length > 0
                  ? 'If a custom model name is not in the list, add it manually. The saved format remains comma-separated.'
                  : 'If the channel does not support discovery or the request fails, enter the model list manually.'
              }
            />
            </div>
            ...
            {manualOnlyModels.length > 0 ? (
              <p className="text-[11px] text-secondary-text">
                Additional manual models: {manualOnlyModels.join('，')}
              </p>
            ) : null}
          </div>
          ...
          <div className="flex items-center gap-2 pt-1">
            ...
            {testState?.text ? (
              <div className="space-y-1">
                ...
                {selectedModels[0] ? (
                  <p className="text-[11px] text-secondary-text">
                    Basic connection test uses the first model in the model list by default: {selectedModels[0]}
                  </p>
                ) : null}
                {testState.hint ? (
                  <p className="text-[11px] text-secondary-text">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2">
              <div>
                ...
                <p className="mt-0.5 text-[11px] text-secondary-text">
                  Real LLM requests are sent only when manually triggered; multiple checks may take 20-40 seconds.
                </p>
              </div>
              <Button
                type="button"
...
function buildModelOptions(models: string[], selectedModel: string, autoLabel: string): Array<{ value: string; label: string }> {
  ...
  if (selectedModel && !models.includes(selectedModel)) {
    options.push({ value: selectedModel, label: `${selectedModel} (current configuration)` });
  }
...
const LLM_TROUBLESHOOTING_HINTS: Record<string, string> = {
  auth: 'Check whether the API key is correct, has extra spaces, or requires extra organization/project permissions for this channel.',
  timeout: 'Retry is possible. If timeouts continue, check Base URL, network proxy, provider region, or local firewall.',
  quota: 'Check balance, plan quota, RPM/TPM limits, or concurrency settings, then retry later if needed.',
  model_not_found: 'Confirm the model name matches the channel protocol, and use Fetch models to check the channel model list.',
  ...
  network_error: 'Check Base URL, proxy, TLS/certificates, gateway, or local network policy, then retry later.',
  invalid_config: 'Complete protocol, Base URL, API key, and model configuration before running a one-click test.',
  unsupported_protocol: 'Automatic model discovery currently supports only OpenAI Compatible / DeepSeek channels. Maintain the model list manually.',
};

const LLM_REASON_HINTS: Record<string, string> = {
  missing_api_key: 'API key is empty, or comma splitting found no usable key. Enter at least one valid key before testing.',
  api_key_rejected: 'The provider rejected the current API key. Check key, organization/project permissions, region, and account status.',
  rate_limit: 'The provider triggered RPM/TPM or concurrency limits. Reduce request frequency or retry later.',
  insufficient_balance: 'The provider reported insufficient balance, billing, or quota. Check account balance and plan status.',
  quota_exceeded: 'The provider reported quota exhaustion. Check account plan, remaining quota, and project quota.',
  provider_blocked: 'The request was blocked by the provider or gateway. Check account risk controls, region restrictions, model permissions, reseller gateway policy, content safety policy, or request origin restrictions.',
  dns_error: 'DNS resolution failed. Check the Base URL domain, network proxy, and DNS configuration.',
  tls_error: 'TLS/certificate handshake failed. Check HTTPS certificates, gateway, or company proxy policy.',
  connection_refused: 'Target service refused the connection. Confirm Base URL port, service process, and firewall configuration.',
  ...
  provider_prefix_mismatch: 'The model provider prefix does not match this channel. Confirm whether the model name should use this channel OpenAI-compatible route.',
  capability_unsupported: 'The current model or compatibility layer does not support this capability. Basic text connectivity is unaffected; switch models or disable this capability dependency.',
};
...
function getLlmTroubleshootingHint(
  ...
  if (code === 'format_error') {
    return context === 'discovery' || stage === 'model_discovery'
      ? 'The channel returned an incompatible /models response format. Enter the model list manually.'
      : 'Response structure does not match expectations. Confirm the channel is compatible with the Chat Completions API.';
  }
  if (code === 'empty_response' && (context === 'discovery' || stage === 'model_discovery')) {
    return 'The channel /models endpoint returned no usable model IDs. Check that Base URL points to a compatible model-list endpoint, or enter models manually.';
  }
...
function buildLlmTestHint(result: {
  ...
  const scopeInfo = 'Basic connection tests only test the first model in the model list by default.';
  ...
  const modelActionHint = shouldSuggestModelListChange
    ? 'If this model is unavailable, adjust model order or remove unavailable models and retry.'
    : '';
...
export const LLMChannelEditor: React.FC<LLMChannelEditorProps> = ({
  ...
  const handleSave = async () => {
    ...
    if (hasEmptyName) {
      setSaveMessage({ type: 'local-error', text: 'Channel name cannot be empty and may contain only letters, numbers, or underscores.' });
      return;
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (mixedPrimary || mixedFallback) {
        setSaveMessage({ type: 'local-error', text: 'Mixed Hermes/non-Hermes routes are not supported as primary or fallback generation models. Choose a pure Hermes or pure non-Hermes route.' });
        return;
      }
      ...
      if (nonCanonicalRouteAlias) {
        setSaveMessage({ type: 'local-error', text: 'The current runtime model uses a non-canonical route alias. Select a canonical model from the dropdown.' });
        return;
      }
    }
    ...
    if (managesRuntimeConfig) {
      ...
      if (invalidPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: 'The current primary model is not in the enabled channel model list. Select another model.' });
        return;
      }
      ...
      if (invalidAgentPrimaryModel) {
        setSaveMessage({ type: 'local-error', text: 'The current Agent primary model has no Agent-safe non-Hermes deployment. Select another model.' });
        return;
      }
      ...
      if (invalidFallbackModel) {
        setSaveMessage({ type: 'local-error', text: 'One or more fallback models are invalid. Select again.' });
        return;
      }
      ...
      if (invalidVisionModel) {
        setSaveMessage({ type: 'local-error', text: 'The current Vision model cannot include a Hermes deployment. Select a pure non-Hermes route.' });
        return;
      }
    }
  ...
  const handleDiscoverModels = async (channel: ChannelConfig) => {
    ...
    try {
      ...
      setDiscoveryStates((previous) => ({
        ...
        [channel.id]: {
          status: 'error',
          text: parsed.message || 'Fetch modelsFailed',
          hint: undefined,
          models: previous[channel.id]?.models || [],
        },
      }));
      ...
      >
        <div className="space-y-1">
          ...
          <p className="text-xs text-muted-text">
            After adding provider channels, models can be fetched and selected automatically; manual entry remains available. Configuration syncs to the .env file.
          </p>
        </div>
      ...
      {!isCollapsed ? (
        <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
          <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
            <div className="mb-3 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-xs text-secondary-text">Select a provider preset first, then create a configuration draft with one click.</p>
              </div>
          ...
          <div className="space-y-2">
            ...
            {channels.length === 0 ? (
              <div className="settings-surface-overlay-muted rounded-[1.35rem] border border-dashed settings-border-strong px-4 py-10 text-center">
                ...
                <p className="mt-1 text-xs text-muted-text">Select a provider preset, then click Add channel to start configuration.</p>
              </div>
          ...
          {managesRuntimeConfig ? (
            <div className="rounded-[1.35rem] border border-[var(--settings-border)] bg-[var(--settings-surface)] p-4 shadow-soft-card">
              <div className="mb-4 flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-[11px] text-muted-text">Primary model, fallback models, Vision, and Temperature are written directly to runtime configuration.</p>
                </div>
              ...
              <div className="mb-4">
                ...
                <p className="mt-1 text-[11px] text-secondary-text">
                  Controls output randomness. 0 is deterministic, 2 is maximum randomness, and 0.7 is recommended.
                </p>
              </div>
              ...
              {availableModels.length === 0 ? (
                <div className="rounded-xl border border-dashed settings-border-strong settings-surface-overlay-soft px-3 py-2 text-xs text-muted-text">
                  Add at least one enabled channel and model before primary, fallback, and Vision options appear below.
                </div>
              ) : (
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(
                        ...
                        'Auto (inherit regular analysis primary model)',
                      )}
                      disabled={busy}
                      placeholder=""
                    />
                  ...
                  <div>
                    ...
                    <Select
                      ...
                      options={buildModelOptions(
                        ...
                        'Auto (follow Vision default logic)',
                      )}
                      disabled={busy}
                      placeholder=""
                    />
            ...
            <InlineAlert
              variant="warning"
              message="Advanced model-routing YAML is configured. This section manages only channel entries and basic connection information. Runtime primary model, fallback models, Vision, and Temperature are still controlled by the common fields below. If YAML parses successfully, its routes and available model declarations take precedence; this configuration does not overwrite the YAML file."
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {!hasChanges ? <span className="text-xs text-muted-text">There are no unsaved changes</span> : null}
          </div>
