  ...
  async enable(): Promise<void> {
    ...
    try {
      ...
      if (!status.available) {
        ...
        throw new Error(`AlphaSift adapter is unavailable${reason}. Confirm that backend project dependencies are installed, and run pip install -r requirements.txt or rebuild the Docker/desktop backend if needed.`);
      }
    } catch (error) {
      try {
