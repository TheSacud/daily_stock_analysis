...
const PortfolioPage: React.FC = () => {
  ...
  const loadBrokers = useCallback(async () => {
    try {
      ...
      if (brokerItems.length === 0) {
        ...
        setBrokerLoadWarning('Broker list API returned empty; using built-in broker list (Huatai/CITIC/CMB).');
      ...
      setBrokers(FALLBACK_BROKERS);
      setBrokerLoadWarning('Broker list API is unavailable; using built-in broker list (Huatai/CITIC/CMB).');
      if (!FALLBACK_BROKERS.some((item) => item.broker === selectedBroker)) {
        setSelectedBroker(FALLBACK_BROKERS[0].broker);
      }
    }
  ...
  const loadSnapshotAndRisk = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        setRiskWarning(parsed.message || 'Failed to fetch risk data; degraded to snapshot-only display.');
      }
    } catch (err) {
      setSnapshot(null);
      setRisk(null);
  ...
  const openAccountDeleteDialog = () => {
    if (!writableAccount) {
      setWriteWarning('Select a specific account before deleting a portfolio account.');
      return;
    }
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setAccountCreateSuccess('Account created successfully and switched automatically.');
    } catch (err) {
  ...
  return (
    <div className="portfolio-page min-h-screen space-y-4 p-4 md:p-6">
        ...
        <Card padding="md">
          <div className="flex items-center justify-between gap-2">
            ...
            {hasAccounts ? (
              ...
              <span className="text-xs text-secondary">Switches to this account after creation</span>
            )}
          </div>
          {accountCreateError ? (
            <InlineAlert
          ...
          <form className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2" onSubmit={handleCreateAccount}>
            ...
            >
              ...
              <option value="jp">Market: Japan (jp)</option>
              <option value="kr">Market: Korea (kr)</option>
              <option value="tw">Market: Taiwan (tw)</option>
            </select>
      ...
      <section className="grid grid-cols-1 xl:grid-cols-3 gap-3">
        <Card padding="md">
          ...
          <form className="space-y-2" onSubmit={handleTradeSubmit}>
            <input className={PORTFOLIO_INPUT_CLASS} placeholder="Stock code (e.g. 600519)" value={tradeForm.symbol}
            ...
            <p className="text-xs text-secondary">Fee and tax can be left empty; the system treats them as 0.</p>
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>Submit trade</button>
          </form>
        </Card>

        <Card padding="md">
          <h3 className="text-sm font-semibold text-foreground mb-3">Manual Entry: Cash Flow</h3>
          <form className="space-y-2" onSubmit={handleCashSubmit}>
            ...
            <button type="submit" className="btn-secondary w-full" disabled={!writableAccountId}>Submit cash flow</button>
          </form>
        </Card>
        ...
        <Card padding="md">
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-2 text-xs text-secondary">
              ...
              <label htmlFor="csv-dry-run">Dry run only (no write)</label>
            </div>
            ...
            {csvParseResult ? (
              <InlineAlert
                ...
                message={`Valid ${csvParseResult.recordCount} rows, skipped ${csvParseResult.skippedCount} rows, errors ${csvParseResult.errorCount} rows.`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
            {csvCommitResult ? (
              <InlineAlert
                ...
                title={csvDryRun ? 'CSV Dry Run Result' : 'CSV Commit Result'}
                message={`${csvDryRun ? 'Dry-run check' : 'Actual write'}: inserted ${csvCommitResult.insertedCount} rows, duplicates ${csvCommitResult.duplicateCount} rows, failed ${csvCommitResult.failedCount} rows.`}
                className="rounded-lg px-3 py-2 text-xs shadow-none"
              />
            ) : null}
          </div>
        ...
        <Card padding="md">
          ...
          <div className="space-y-2">
            ...
            {(eventType === 'trade' || eventType === 'corporate') ? (
              <input className={PORTFOLIO_INPUT_CLASS} placeholder="Filter by stock code" value={eventSymbol}
            ...
            <div className="text-[11px] text-secondary">
              {writeBlocked ? 'Delete correction is available only in single-account view. Select a specific account before deleting incorrect events.' : 'If an event is wrong, delete it and enter it again.'}
            </div>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'trade',
                        id: item.id,
                        message: `Confirm deleting ${item.tradeDate} ${formatSideLabel(item.side)} event ${item.symbol}(quantity ${item.quantity} price ${item.price}?`,
                      })}
                    >
                      Delete
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'cash',
                        id: item.id,
                        message: `Confirm deleting ${item.eventDate} Cash flows（${formatCashDirectionLabel(item.direction)} ${item.amount} ${item.currency}?`,
                      })}
                    >
                      Delete
                    </button>
                  ...
                  {!writeBlocked ? (
                    <button
                      ...
                      onClick={() => openDeleteDialog({
                        eventType: 'corporate',
                        id: item.id,
                        message: `Confirm deleting ${item.effectiveDate} Corporate actions ${formatCorporateActionLabel(item.actionType)}（${item.symbol}?`,
                      })}
                    >
                      Delete
                    </button>
              ...
              {!eventLoading
                && ((eventType === 'trade' && tradeEvents.length === 0)
                  ...
                  || (eventType === 'corporate' && corporateEvents.length === 0)) ? (
                    <EmptyState
                      title="No events"
                      description="Adjust filters or enter a trade, cash flow, or corporate action first."
      ...
      <ConfirmDialog
        ...
        message={pendingDelete?.message || 'Confirm deleting this event?'}
