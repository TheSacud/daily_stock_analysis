...
function getConfidenceMeta(confidence: 'high' | 'medium' | 'low', language: UiLanguage) {
  if (confidence === 'high') {
    return { label: language === 'en' ? 'High' : 'High', badge: 'success' as const };
  }
  if (confidence === 'low') {
    return { label: language === 'en' ? 'Low' : 'Low', badge: 'warning' as const };
  }
  return { label: language === 'en' ? 'Medium' : 'Medium', badge: 'default' as const };
}
