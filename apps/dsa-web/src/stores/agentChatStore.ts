    ...
    try {
      ...
      const processLine = (line: string) => {
        ...
        if (event.type === 'done') {
          ...
          if (doneEvent.success === false) {
            throw getStreamFailureError(doneEvent, 'LLM call failed. Check the API key configuration.');
          }
      ...
      if (!receivedDoneEvent && !ac.signal.aborted) {
        throw createParsedApiError({
          title: 'Reply was incomplete',
          message: 'The agent streaming response ended before completion. Please retry.',
