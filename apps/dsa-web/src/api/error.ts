...
export function parseApiError(error: unknown): ParsedApiError {
  ...
  if (errorCode === 'portfolio_oversell' || includesAny(matchText, ['oversell detected'])) {
    return createParsedApiError({
      ...
      message: 'The sell quantity exceeds the available position. Delete or correct the corresponding sell transaction and try again.',
      rawMessage,
      status,
  ...
  if (errorCode === 'portfolio_busy' || includesAny(matchText, ['portfolio ledger is busy'])) {
    return createParsedApiError({
      ...
      message: 'The portfolio ledger is processing another change. Try again later.',
      rawMessage,
      status,
  ...
  if (errorCode === 'alphasift_install_failed') {
    return createParsedApiError({
      title: 'AlphaSift repair installation failed',
      message: 'DSA attempted to repair-install AlphaSift, but pip installation failed. Check ALPHASIFT_INSTALL_SPEC, network proxy, or the backend Python environment.',
      rawMessage,
      status,
      category: 'http_error',
    });
  ...
  if (errorCode === 'alphasift_install_spec_missing') {
    return createParsedApiError({
      ...
      message: 'Confirm backend dependencies are installed first. To use repair installation, configure a trusted ALPHASIFT_INSTALL_SPEC.',
      rawMessage,
      status,
      category: 'http_error',
    });
  ...
  if (errorCode === 'alphasift_install_spec_not_allowed') {
    return createParsedApiError({
      ...
      message: 'Repair installation only allows trusted AlphaSift GitHub sources. For local paths or wheels, install manually into the current Python environment first.',
      rawMessage,
      status,
      category: 'http_error',
    });
  ...
  if (errorCode === 'alphasift_adapter_unavailable') {
    return createParsedApiError({
      ...
      message: 'The current AlphaSift version is missing the stable DSA adapter. Reinstall or upgrade AlphaSift and try again.',
      category: 'http_error',
      rawMessage,
      status,
    });
  ...
  if (errorCode === 'alphasift_screen_task_not_found') {
    return createParsedApiError({
      ...
      message: 'The server could not find this screening task. The backend may have restarted or task records may have been cleared. Run screening again.',
      rawMessage,
      status,
      category: 'http_error',
    });
  ...
  if (errorCode === 'alphasift_screen_failed') {
    return createParsedApiError({
      ...
      message: 'AlphaSift failed while accessing external market, snapshot, or model services. Try again later or check network and proxy settings.',
      rawMessage,
      status,
  ...
  if (noConfiguredLlm) {
    return createParsedApiError({
      title: 'No available LLM model is configured',
  ...
  if (includesAny(matchText, [
    ...
    return createParsedApiError({
      title: 'Current model is not compatible with tool calls',
  ...
  if (includesAny(matchText, [
    ...
    return createParsedApiError({
      ...
      message: 'The upstream model returned a tool-call structure that does not meet requirements. Switch models or disable the related reasoning mode and try again.',
      rawMessage,
      status,
  ...
  if (includesAny(matchText, ['timeout', 'timed out', 'read timeout', 'connect timeout']) || code === 'ECONNABORTED') {
    return createParsedApiError({
      title: 'Connection to upstream service timed out',
  ...
  if (
    ...
    return createParsedApiError({
      title: 'Server cannot access external dependencies',
      message: 'The page is connected to the local service, but the local service cannot access external model or data APIs. Check proxy, DNS, or outbound network configuration.',
      rawMessage,
      status,
      category: 'upstream_network',
    });
  ...
  if (status === 400 && hasLlmProviderHint) {
    return createParsedApiError({
      ...
      message: 'The local service is healthy, but the upstream model API rejected the request. Check model name, parameter format, or tool-call compatibility.',
      rawMessage,
      status,
  ...
  if (localConnectionFailed) {
    return createParsedApiError({
      ...
      message: 'The browser cannot connect to the local Web service. Check whether the service is running, listening on the correct address, and exposing the port.',
      rawMessage,
      status,
